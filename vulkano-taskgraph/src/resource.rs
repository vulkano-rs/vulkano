...
use crate::{Id, InvalidSlotError, Ref};
use ash::vk;
...
impl Resources {
    ...
    pub fn new(
        ...
        Resources {
            memory_allocator,
            global: epoch::GlobalHandle::new(),
            locals: ThreadLocal::new(),
            buffers: SlotMap::new(create_info.max_buffers),
            images: SlotMap::new(create_info.max_images),
            swapchains: SlotMap::new(create_info.max_swapchains),
            flights: SlotMap::new(create_info.max_flights),
        }
    }
    ...
    pub fn create_swapchain(
        ...
        assert_eq!(surface.instance(), self.device().instance());
        ...
        Ok(unsafe { self.add_swapchain_unchecked(swapchain, images, flight_id) }?)
    }
    ...
    pub fn create_flight(&self, frame_count: u32) -> Result<Id<Flight>, VulkanError> {
        ...
        Ok(Id::new(self.flights.insert(flight, self.pin())))
    }
    ...
    unsafe fn add_buffer_unchecked(&self, buffer: Arc<Buffer>) -> Id<Buffer> {
        ...
        Id::new(self.buffers.insert(state, self.pin()))
    }
    ...
    unsafe fn add_image_unchecked(&self, image: Arc<Image>) -> Id<Image> {
        ...
        Id::new(self.images.insert(state, self.pin()))
    }
    ...
    /// # Panics
    ///
    /// - Panics if any other references to the swapchain exist.
    ...
    pub fn add_swapchain(
        ...
        unsafe { self.add_swapchain_unchecked(swapchain, images, flight_id) }
    }
    ...
    unsafe fn add_swapchain_unchecked(
        ...
        let id = Id::new(self.swapchains.insert(state, guard));
    ...
    /// Removes the buffer corresponding to `id`, or returns `None` if it isn't present.
    ///
    /// # Safety
    ///
    ...
    /// Removes the image corresponding to `id`, or returns `None` if it isn't present.
    ///
    /// # Safety
    ///
    ...
    /// Removes the swapchain corresponding to `id`, or returns `None` if it isn't present.
    ///
    /// # Safety
    ///
    ...
    /// Returns the buffer corresponding to `id`, or returns `None` if it isn't present.
    #[inline]
    ...
    /// Returns the image corresponding to `id`, or returns `None` if it isn't present.
    #[inline]
    ...
    /// Returns the swapchain corresponding to `id`, or returns `None` if it isn't present.
    #[inline]
    ...
    /// Returns the [flight] corresponding to `id`, or returns `None` if it isn't present.
    #[inline]
    #[must_use]
    pub fn flight(&self, id: Id<Flight>) -> Option<Ref<'_, Flight>> {
        self.flights.get(id.slot, self.pin()).map(Ref)
    }

    #[inline]
    pub(crate) unsafe fn flight_unprotected(&self, id: Id<Flight>) -> Option<&Flight> {
        // SAFETY: Enforced by the caller.
        unsafe { self.flights.get_unprotected(id.slot) }
    }
...
impl ImageAccess {
    ...
    pub const fn new(
        ...
        if matches!(access_type.image_layout(), ImageLayout::General) {
            layout_type = ImageLayoutType::General;
        }
...
access_types! {
    ...
    AccelerationStructureCopyTrasferWrite {
        stage_mask: ACCELERATION_STRUCTURE_COPY,
        access_mask: TRANSFER_WRITE,
        image_layout: Undefined,
    }
...
pub enum ImageLayoutType {
    ...
    /// time.
    ///
    /// The optimal layout depends on the access. For instance, for color attachment output, the
    /// only valid optimal layout is [`ColorAttachmentOptimal`]. For transfer sources, it's
    /// [`TransferSrcOptimal`], and so on. Some accesses don't have an optimal layout for them. In
    /// such cases, using this option makes no difference, as the [general layout] will always be
    /// used.
    ///
