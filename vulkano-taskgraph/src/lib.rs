...
use vulkano::{
    ...
    memory::{
        ...
        DeviceAlignment, MappedMemoryRange, MemoryPropertyFlags, ResourceMemory,
    },
    swapchain::Swapchain,
    DeviceSize, ValidationError, VulkanError, VulkanObject,
};
...
pub trait Task: Any + Send + Sync {
    ...
    unsafe fn execute(&self, ctx: &mut TaskContext<'_>) -> TaskResult;
}
...
impl dyn Task {
    ...
    #[inline]
    pub unsafe fn downcast_unchecked_ref<T: Task>(&self) -> &T {
        // SAFETY: The caller must guarantee that the type is correct.
        unsafe { &*<*const dyn Task>::cast::<T>(self) }
    }
    ...
    /// `self` must be of type `T`.
    #[inline]
    pub unsafe fn downcast_unchecked_mut<T: Task>(&mut self) -> &mut T {
        // SAFETY: The caller must guarantee that the type is correct.
        unsafe { &mut *<*mut dyn Task>::cast::<T>(self) }
    }
}
...
impl<'a> TaskContext<'a> {
    ...
    ///
    /// This function behaves identically to the [`push_command_buffer`] method, execept that it
    ...
    pub fn read_buffer<T: BufferContents + ?Sized>(
        ...
        unsafe fn invalidate_subbuffer(
            ...
            ctx.validate_read_buffer(subbuffer.buffer(), range.clone())?;
    ...
    fn validate_read_buffer(&self, buffer: &Buffer, range: BufferRange) -> TaskResult {
        todo!()
    }
    ...
    pub fn write_buffer<T: BufferContents + ?Sized>(
        ...
        unsafe fn invalidate_subbuffer(
            ...
            ctx.validate_write_buffer(subbuffer.buffer(), range.clone())?;
    ...
    fn validate_write_buffer(&self, buffer: &Buffer, range: BufferRange) -> TaskResult {
        todo!()
    }
...
impl<T: ?Sized> Drop for BufferWriteGuard<'_, T> {
    ...
    fn drop(&mut self) {
        ...
        fn flush_subbuffer(subbuffer: &Subbuffer<[u8]>, atom_size: DeviceAlignment) {
            ...
            let atom_size = allocation.atom_size().unwrap();
...
impl fmt::Display for TaskError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let msg = match self {
            ...
            Self::HostAccess(_) => "a host access error occured",
            Self::ValidationError(_) => "a validation error occured",
        };
...
impl InvalidSlotError {
    fn new<O: VulkanObject>(id: Id<O>) -> Self {
        InvalidSlotError {
...
impl fmt::Display for InvalidSlotError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let &InvalidSlotError { object_type, slot } = self;
...
///
/// This loses you the downcasting capabilities, but that should be fine for most use cases. Using
/// a closure is much more ergonomic than having to define a new type for each kind of task.
#[derive(Clone, Copy)]
pub struct AdHocTask<F>(pub F);

impl<F> From<F> for AdHocTask<F> {
    #[inline]
    fn from(f: F) -> Self {
        AdHocTask(f)
    }
...
impl<F> fmt::Debug for AdHocTask<F> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("AdHoc").finish_non_exhaustive()
    }
}

impl<F> Task for AdHocTask<F>
where
    F: Fn(&mut TaskContext<'_>) -> TaskResult + Send + Sync + 'static,
{
    unsafe fn execute(&self, ctx: &mut TaskContext<'_>) -> TaskResult {
        (self.0)(ctx)
    }
