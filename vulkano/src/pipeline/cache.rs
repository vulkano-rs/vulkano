...
impl PipelineCache {
    ...
    /// ```
    #[inline]
    pub unsafe fn with_data(device: &Arc<Device>, initial_data: &[u8]) -> Result<Arc<PipelineCache>, OomError> {
    ...
    unsafe fn new_impl(device: &Arc<Device>, initial_data: Option<&[u8]>) -> Result<Arc<PipelineCache>, OomError> {
        ...
        let cache = {
            ...
            try!(check_errors(vk.CreatePipelineCache(device.internal_object(), &infos, ptr::null(), &mut output)));
            output
        };
        ...
        where I: IntoIterator<Item = &'a &'a Arc<PipelineCache>>
    {
        unsafe {
            ...
            try!(check_errors(vk.MergePipelineCaches(self.device.internal_object(),
                                                     self.cache,
    ...
    pub fn get_data(&self) -> Result<Vec<u8>, OomError> {
        unsafe {
            ...
            let mut num = 0;
            try!(check_errors(vk.GetPipelineCacheData(self.device.internal_object(),
                                                      self.cache,
                                                      &mut num,
                                                      ptr::null_mut())));
