...
struct PhysicalDeviceHostImageCopyPropertiesEXTFields {
    copy_src_layouts: Vec<ash::vk::ImageLayout>,
    copy_dst_layouts: Vec<ash::vk::ImageLayout>,
}

pub(crate) struct DevicePropertiesFieldsVk {
    properties_host_image_copy_ext: Option<PhysicalDeviceHostImageCopyPropertiesEXTFields>,
}

impl DevicePropertiesFieldsVk {
    pub fn is_empty(&self) -> bool {
        self.properties_host_image_copy_ext.is_none()
    }
}

impl DevicePropertiesFfi {
    pub(crate) fn fields(&mut self) -> DevicePropertiesFieldsVk {
        DevicePropertiesFieldsVk {
            properties_host_image_copy_ext: self.properties_host_image_copy_ext.as_mut().map(|it| {
                let mut fields = PhysicalDeviceHostImageCopyPropertiesEXTFields {
                    copy_dst_layouts: vec![Default::default(); it.copy_dst_layout_count as usize],
                    copy_src_layouts: vec![Default::default(); it.copy_src_layout_count as usize],
                };

                it.p_copy_dst_layouts = fields.copy_dst_layouts.as_mut_ptr();
                it.p_copy_src_layouts = fields.copy_src_layouts.as_mut_ptr();
