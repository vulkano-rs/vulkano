...
impl Device {
    ...
    pub unsafe fn from_handle(
        ...
        unsafe { Self::from_handle_impl(physical_device, handle, create_info, false) }
    }
    ...
    /// - `handle` must be a valid Vulkan object handle created from `physical_device`.
    /// - `create_info` must match the info used to create the object.
    /// - `handle` must not be freed while the returned `Device` object is still alive. This means all
    ///   copies of the returned `Arc` must be dropped first. Note `DeviceOwned` objects all hold a
    ///   reference to the device internally.
    pub unsafe fn from_handle_borrowed(
        physical_device: Arc<PhysicalDevice>,
        handle: vk::Device,
        create_info: DeviceCreateInfo,
    ) -> (Arc<Device>, impl ExactSizeIterator<Item = Arc<Queue>>) {
        unsafe { Self::from_handle_impl(physical_device, handle, create_info, true) }
    }
