          ...
          S: TrackedDescriptorSetsCollection
{
    ...
    pub fn new(previous: L, pipeline: Arc<ComputePipeline<Pl>>, sets: S, dimensions: [u32; 3], push_constants: Pc) -> Result<CmdDispatch<L, Pl, S, Pc>, CmdDispatchError> {
...
impl error::Error for CmdDispatchError {
    ...
    fn description(&self) -> &str {
        match *self {
            CmdDispatchError::DimensionsTooLarge => "the dispatch dimensions are larger than the hardware limits",
            CmdDispatchError::BindDescriptorSetsError(_) => "error while binding descriptor sets",
            CmdDispatchError::PushConstantsError(_) => "error while setting push constants",
        }
    }
