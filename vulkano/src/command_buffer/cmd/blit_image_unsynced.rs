...
impl<L, S, D> CmdBlitImageUnsynced<L, S, D>
    where L: CommandsList,
          ...
          D: Image
{
    pub unsafe fn new<I>(previous: L, source: S, source_layout: Layout, destination: D, destination_layout: Layout, filter: Filter, regions: I) -> Result<CmdBlitImageUnsynced<L, S, D>, CmdBlitImageUnsyncedError>
        where I: IntoIterator<Item = BlitRegion>
    {
        ...
        debug_assert!(source_layout == Layout::TransferSrcOptimal || source_layout == Layout::General);
        debug_assert!(destination_layout == Layout::TransferDstOptimal || destination_layout == Layout::General);
...
impl error::Error for CmdBlitImageUnsyncedError {
    ...
    fn description(&self) -> &str {
        match *self {
            CmdBlitImageUnsyncedError::SourceUsageMissingSrcTransfer => "the source image wasn't created with source transfer operations support",
            CmdBlitImageUnsyncedError::DestinationUsageMissingDestTransfer => "the destination image wasn't created with destination transfer operations support",
            CmdBlitImageUnsyncedError::SourceFormatDoesntSupportBlits => "the format of the source image doesn't support blit operations",
            CmdBlitImageUnsyncedError::DestinationFormatDoesntSupportBlits => "the format of the destination image doesn't support blit operations",
            CmdBlitImageUnsyncedError::IncompatibleSourceDestinationFormats => "the format of the source and the destination aren't compatible for blit operations",
            CmdBlitImageUnsyncedError::SourceMultisampled => "the source image must only have one sample",
            CmdBlitImageUnsyncedError::DestinationMultisampled => "the destination image must only have one sample",
            CmdBlitImageUnsyncedError::SourceDoesntSupportLinearFilter => "the filter is `Linear` but the source format doesn't support linear filtering",
        }
    }
}
