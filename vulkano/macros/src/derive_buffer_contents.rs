...
pub fn derive_buffer_contents(mut ast: DeriveInput) -> Result<TokenStream> {
    ...
    {
        bail!(
            "deriving `BufferContents` is only supported for types that are marked `#[repr(C)]` or 
            `#[repr(transparent)]`",
        );
    }
    ...
    Ok(quote! {
        ...
        unsafe impl #impl_generics ::#crate_ident::buffer::subbuffer::BufferContents for #struct_ident #type_generics #where_clause {
            ...
            unsafe fn from_ffi(data: *const ::std::ffi::c_void, range: usize) -> *const Self {
                ...
                // unsized, must therefore be a slice or a DST derived from a slice. It can not be
                ...
                // pointer to the type will by definition be smaller, and since transmuting types
                // of different sizes never works, it will cause a compilation error on this line.
                //
