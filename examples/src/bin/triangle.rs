...
fn main() {
    ...
    // each image.
    //
    // The only reason this variable is mutable is because we may have to recreate it again
    // later (you'll see that further below).
    ...
    loop {
        ...
        if recreate_swapchain {
            use std::mem::replace;
            let (new_swapchain, new_images) = swapchain.recreate_with_dimension([1280,1024])
                .unwrap();
        ...
        let image_num = match swapchain.acquire_next_image(Duration::new(1, 0)) {
            Ok(img) => img,
            Err(AcquireError::OutOfDate) => {
                // In this case, the swapchain needs to be recreated.  We make a note of it
                // and restart the loop to make it happen.
        ...
        match swapchain.present(&queue, image_num) {
            Ok(()) => (),
            Err(PresentError::OutOfDate) => {
                // In this case, the swapchain needs to be recreated.  We make a note of it.
                recreate_swapchain = true;
                // Since we are already at the bottom of the loop, we just let it wrap around.
            },
            Err(err) => panic!("{:?}", err)
        }
