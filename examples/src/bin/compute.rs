...
fn main() {
    ...
    // We will start with a simple data buffer (packed_input) filled with 32bpp RGBA values. This should be similar to many
    // common image processing problems, but simplified to show the concepts only. From the packed_input we will compute a
    // regular vulkan texture object (uimage2d) that can be used in rendering logic which will not be shown here. Sometimes
    ...
    // Create an image to store the pixel values we compute (this will be the output of step 1 and the input to step 2)
    let image_buffer = StorageImage::new(&device, Dimensions::Dim2d { width: image_width, height: image_height }, Format::R8G8B8A8Uint, Some(queue.family())).unwrap();

    {
        ...
        let set0 = pipeline_mod::set0::Set::new(&descriptor_pool, &pipeline, &pipeline_mod::set0::Descriptors {
            packed_input: &packed_input
        });
        let set1 = pipeline_mod::set1::Set::new(&descriptor_pool, &pipeline, &pipeline_mod::set1::Descriptors {
            output_image: &image_buffer
        });
        let pipeline = ComputePipeline::new(&device, &pipeline, &shader.main_entry_point(), &()).unwrap();
    ...
    let packed_output = CpuAccessibleBuffer::<[u8]>::array(&device, byte_len, &BufferUsage::all(), Some(queue.family())).expect("failed to create buffer");
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Note: At this point packed_output has been filled with the same values we started from: 0, 1, 2, 3...     //
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
