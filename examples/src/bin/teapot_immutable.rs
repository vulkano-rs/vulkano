...
fn main() {
    ...
    let (vertex_buffer, vertex_upload_future) = vulkano::buffer::immutable::ImmutableBuffer::from_iter(
        examples::VERTICES.iter().cloned(), 
        vulkano::buffer::BufferUsage::all(), 
        queue.clone()).expect("Failed to create vertex buffer");

    let (normals_buffer, normals_upload_future) = vulkano::buffer::immutable::ImmutableBuffer::from_iter(
        examples::NORMALS.iter().cloned(), 
        vulkano::buffer::BufferUsage::all(), 
        queue.clone()).expect("Failed to create normals buffer");

    let (index_buffer, index_upload_future) = vulkano::buffer::immutable::ImmutableBuffer::from_iter(
        examples::INDICES.iter().cloned(), 
        vulkano::buffer::BufferUsage::all(), 
        queue.clone()).expect("Failed to create index buffer");

    // Join the vertex, normals, and index futures into a single future for convience
    let data_upload_future = vertex_upload_future.join(normals_upload_future).join(index_upload_future);

    let fence_signal = Arc::new(data_upload_future.then_signal_fence());

    // Wait for the data upload to be complete, for simplicity of the example.
    // 
    // note: This could be done more efficiently. Instead of waiting here we could signal the GPU
    //       using synchronization primitives to only draw once the data has been uploaded.  
