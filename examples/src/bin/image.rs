...
fn main() {
    ...
    let vertex_buffer = vulkano::buffer::cpu_access::CpuAccessibleBuffer::<[Vertex]>::array(&device,
                                                                                            4,
    ...
    mod vs {
        include!{concat!(env!("OUT_DIR"), "/shaders/src/bin/image_vs.glsl")}
    }
    ...
    let renderpass = renderpass::CustomRenderPass::new(&device,
                                                       &renderpass::Formats { color: (vulkano::format::B8G8R8A8Srgb, 1) })
        .unwrap();

    let texture = vulkano::image::immutable::ImmutableImage::new(&device,
    ...
    let pixel_buffer = {
        let image = image::load_from_memory_with_format(include_bytes!("image_img.png"), image::ImageFormat::PNG)
            .unwrap()
            .to_rgba();
        let image_data = image.into_raw().clone();

        // TODO: staging buffer instead
        let pixel_buffer = vulkano::buffer::cpu_access::CpuAccessibleBuffer::<[[u8; 4]]>::array(&device,
                                                                                                image_data.len(),
                                                                                                &vulkano::buffer::BufferUsage::all(),
                                                                                                Some(queue.family()))
            .expect("failed to create buffer");
    ...
    let sampler = vulkano::sampler::Sampler::new(&device,
    ...
    let set = pipeline_layout::set0::Set::new(&descriptor_pool,
                                              &pipeline_layout,
                                              &pipeline_layout::set0::Descriptors { tex: (&sampler, &texture) });
    ...
    let framebuffers = images.iter()
        .map(|image| {
            ...
            vulkano::framebuffer::Framebuffer::new(&renderpass,
                                                   [images[0].dimensions()[0], images[0].dimensions()[1], 1],
                                                   attachments)
                .unwrap()
        })
        .collect::<Vec<_>>();
