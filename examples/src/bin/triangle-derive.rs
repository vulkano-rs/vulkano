...
fn main() {
    ...
    //
    // We derive `Vertex` to implement the `Vertex` trait using a procedural macro.
    // There are two field-level attributes:
    // - `format` specifies the `vulkano::format::Format` to use for the shader input attribute.
    // - `name` can override the associated shader input attribute name that is matched.
    // If no field-level attributes are implemented the field's type has to implement
    // `VertexMember` and the field-name is used to match the shader input.
    #[repr(C)]
