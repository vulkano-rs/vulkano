...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
            ...
            self.resources
                .create_swapchain(
                    ...
                    &SwapchainCreateInfo {
                        ...
                        // `MAX_FRAMES_IN_FLIGHT + 1` into account. We need at least as many images
                        // as frames in flight, otherwise not all frames would actually be in
                        // flight since there wouldn't be enough images for the device to work on
                        // at the same time. At least, that would be the case if only the host and
                        // device were involved. When it comes to presentation, there is a third
                        // party involved: the "Presentation Engine". It can be working on one
                        // swapchain image at a time, and its work is not necessarily in sync with
                        // the device, so we need one more swapchain image. With fewer swapchain
                        // images, we could be blocking on the host while acquiring the next image.
        ...
        // for a physical framebuffer. For one, the task graph creates its framebuffers and render
        // passes internally, so you don't need to specify these yourself. Also, a single virtual
        // framebuffer doesn't necessarily correspond to a single physical framebuffer. Instead,
        // all a virtual framebuffer is for is a way to inform the task graph that different nodes'
        // attachments have the same dimensions. This allows the task graph to combine different
        // nodes into the same render pass, or even the same subpass. Therefore, you should use the
        // same virtual framebuffer across nodes that share the same framebuffer dimensions.
