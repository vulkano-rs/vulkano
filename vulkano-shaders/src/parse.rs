...
fn decode_instruction(opcode: u16, operands: &[u32]) -> Result<Instruction, ParseError> {
    Ok(match opcode {
        ...
        14 => {
            Instruction::MemoryModel(try!(AddressingModel::from_num(operands[0])),
                                     try!(MemoryModel::from_num(operands[1])))
        }
        15 => {
        ...
        25 => {
            Instruction::TypeImage {
                ...
                access: if operands.len() >= 9 {
                    Some(try!(AccessQualifier::from_num(operands[8])))
                } else {
                    None
                },
            }
        ...
        32 => {
            Instruction::TypePointer {
                result_id: operands[0],
                storage_class: try!(StorageClass::from_num(operands[1])),
                type_id: operands[2],
            }
        }
        43 => {
        ...
        71 => {
            Instruction::Decorate {
                target_id: operands[0],
                decoration: try!(Decoration::from_num(operands[1])),
        ...
        72 => {
            Instruction::MemberDecorate {
                target_id: operands[0],
                member: operands[1],
                decoration: try!(Decoration::from_num(operands[2])),
