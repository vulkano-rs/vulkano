...
fn write_struct(doc: &parse::Spirv, struct_id: u32, members: &[u32]) -> String {
    ...
    for (num, &member) in members.iter().enumerate() {
        ...
        let spirv_offset = doc.instructions
            ...
            .filter_map(|i| {
                match *i {
                    parse::Instruction::MemberDecorate { target_id, member, decoration: enums::Decoration::DecorationOffset, ref params } if target_id == struct_id && member as usize == num => {
    ...
    let spirv_req_total_size = doc.instructions
        ...
        .filter_map(|i| {
            match *i {
                parse::Instruction::Decorate { target_id, decoration: enums::Decoration::DecorationArrayStride, ref params } => {
                    for inst in doc.instructions.iter() {
                        match *inst {
                            parse::Instruction::TypeArray { result_id, type_id, .. } if result_id == target_id && type_id == struct_id => {
                                return Some(params[0]);
                            }
                            parse::Instruction::TypeRuntimeArray { result_id, type_id } if result_id == target_id && type_id == struct_id => {
                                return Some(params[0]);
                            }
                            _ => (),
                        }
...
fn is_builtin_member(doc: &parse::Spirv, id: u32, member_id: u32) -> bool {
    for instruction in &doc.instructions {
        match *instruction {
            parse::Instruction::MemberDecorate { target_id, member, decoration: enums::Decoration::DecorationBuiltIn, .. } if target_id == id && member == member_id => {
                return true;
            }
            _ => (),
        }
