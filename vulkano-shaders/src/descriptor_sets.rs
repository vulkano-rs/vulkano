...
pub fn write_descriptor_sets(doc: &parse::Spirv) -> String {
    ...
    for instruction in doc.instructions.iter() {
        let (variable_id, descriptor_set) = match instruction {
            &parse::Instruction::Decorate { target_id, decoration: enums::Decoration::DecorationDescriptorSet, ref params } => (target_id, params[0]),
            _ => continue,
        };
        ...
        let binding = doc.instructions
            ...
            .filter_map(|i| {
                match i {
                    &parse::Instruction::Decorate { target_id, decoration: enums::Decoration::DecorationBinding, ref params } if target_id == variable_id => Some(params[0]),
        ...
        let (desc_ty, readonly) = descriptor_infos(doc, pointed_ty, false).expect(&format!("Couldn't find relevant type for uniform `{}` (type {}, maybe unimplemented)",
                                                                                           name,
                                                                                           pointed_ty));
    ...
    let descriptor_body = descriptors.iter()
        .map(|d| {
    ...
    let descriptor_by_name_body = descriptors.iter()
        .map(|d| {
...
fn pointer_variable_ty(doc: &parse::Spirv, variable: u32) -> u32 {
    let var_ty = doc.instructions
        ...
        .filter_map(|i| {
            match i {
                &parse::Instruction::Variable { result_type_id, result_id, .. } if result_id == variable => Some(result_type_id),
                _ => None,
            }
        })
        .next()
    ...
    doc.instructions
        ...
        .filter_map(|i| {
            match i {
                &parse::Instruction::TypePointer { result_id, type_id, .. } if result_id == var_ty => Some(type_id),
                _ => None,
            }
        })
        .next()
...
fn descriptor_infos(doc: &parse::Spirv, pointed_ty: u32, force_combined_image_sampled: bool) -> Option<(String, bool)> {
    doc.instructions
        .iter()
        .filter_map(|i| {
            match i {
                &parse::Instruction::TypeStruct { result_id, .. } if result_id == pointed_ty => {
                    ...
                    let is_ssbo = doc.instructions
                        .iter()
                        .filter_map(|i| {
                            match i {
                                &parse::Instruction::Decorate { target_id, decoration: enums::Decoration::DecorationBufferBlock, .. } if target_id == pointed_ty => Some(true),
                                &parse::Instruction::Decorate { target_id, decoration: enums::Decoration::DecorationBlock, .. } if target_id == pointed_ty => Some(false),
                                _ => None,
                            }
                        })
                        .next()
                        .expect("Found a buffer uniform with neither the Block nor BufferBlock decorations");
                ...
                &parse::Instruction::TypeImage { result_id, ref dim, arrayed, ms, sampled, ref format, .. } if result_id == pointed_ty => {
                    let sampled = sampled.expect("Vulkan requires that variables of type OpTypeImage have a Sampled operand of 1 or 2");
                    ...
                    if let &enums::Dim::DimSubpassData = dim {
                        ...
                        assert!(!force_combined_image_sampled,
                                "An OpTypeSampledImage can't point to an OpTypeImage whose dimension is SubpassData");
                        assert!(if let &enums::ImageFormat::ImageFormatUnknown = format {
                                    true
                                } else {
                                    false
                                },
                        ...
                        let ty = if force_combined_image_sampled {
                ...
                &parse::Instruction::TypeSampledImage { result_id, image_type_id } if result_id == pointed_ty => descriptor_infos(doc, image_type_id, true),
